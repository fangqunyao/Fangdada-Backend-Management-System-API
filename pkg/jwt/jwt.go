...
func GenerateTokenByAdmin(admin entity.SysAdmin) (string, error) {
	...
	c := userStdClaims{
		jwtAdmin, // 自定义字段
		jwt.RegisteredClaims{
			ExpiresAt: &jwt.NumericDate{Time: time.Now().Add(TokenExpireDuration)}, // 使用 NumericDate
...
func GetAdminName(c *gin.Context) (string, error) {
	u, exist := c.Get(constant.ContextKeyUserObj)
	if !exist {
		return string(strconv.Itoa(0)), errors.New("can't get user name")
	}
	admin, ok := u.(*entity.JwtAdmin)
	if ok {
		return admin.Username, nil
	}
	return string(strconv.Itoa(0)), errors.New("can't convert to api name")
}
