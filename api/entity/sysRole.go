...
type SysRole struct {
	...
	RoleKey     string    `gorm:"column:role_key;varchar(64);comment:'权限字符串'" json:"roleKey"`                  // 权限字符串
	...
	CreateTime  time.Time `gorm:"column:create_time;comment:'创建时间';NOT NULL" json:"createTime"`                // 创建时间
}
...
type AddSysRoleDto struct {
	...
	RoleKey     string `json:"roleKey"` // 可选
...
type SysRoleIdDto struct {
	Id uint `form:"id" binding:"required"` // ✅ 用于 query/form 绑定
}
